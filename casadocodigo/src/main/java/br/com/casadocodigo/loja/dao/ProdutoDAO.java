...
public class ProdutoDAO {
	...
	public List<Produto> filtrar(Calendar dataLancamento) {

		StringBuilder hql = new StringBuilder("select distinct(p) from Produto p join fetch p.precos ");

		if (dataLancamento != null) {
			hql.append("where p.dataLancamento >= :dataFiltro");
		}

		TypedQuery<Produto> query = manager.createQuery(hql.toString(), Produto.class);

		if (dataLancamento != null) {
			query.setParameter("dataFiltro", dataLancamento);
		}
